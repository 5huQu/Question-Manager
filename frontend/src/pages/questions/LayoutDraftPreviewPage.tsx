...
export default function LayoutDraftPreviewPage() {
  ...
  async function exportPdf() {
    if (!draft) return
    setBusy(true)
    try { const result = await layoutDraftsApi.export(draft.id, draft.revision); if (result.url) window.open(result.url, '_blank') }
    catch (cause) { setError(cause instanceof Error ? cause.message : String(cause)) }
    finally { setBusy(false) }
  }
  ...
  return <div className="space-y-4">
    <div className="flex items-center justify-between">
      ...
      <div className="flex gap-2"><div className="flex rounded-md border bg-zinc-100 p-0.5 dark:bg-zinc-900" role="tablist" aria-label="预览版本"><button className={`rounded px-3 py-1.5 text-xs ${variant === 'student' ? 'bg-white font-medium shadow-sm dark:bg-zinc-800' : ''}`} onClick={() => setVariant('student')}>学生版</button><button className={`rounded px-3 py-1.5 text-xs ${variant === 'teacher' ? 'bg-white font-medium shadow-sm dark:bg-zinc-800' : ''}`} onClick={() => setVariant('teacher')}>教师版</button></div><button className="rounded-md border px-3 py-2 text-xs" title="缩小" onClick={() => setZoom(Math.max(40, zoom - 10))}><ZoomOut className="size-4"/></button><button className="rounded-md border px-3 py-2 text-xs" title="放大" onClick={() => setZoom(Math.min(140, zoom + 10))}><ZoomIn className="size-4"/></button><button disabled={busy} className="inline-flex items-center gap-2 rounded-md border px-3 py-2 text-xs" onClick={() => void preview()}><RefreshCw className={`size-4 ${busy ? 'animate-spin' : ''}`}/>生成 PDF</button><button disabled={busy || !draft} className="inline-flex items-center gap-2 rounded-md bg-zinc-950 px-3 py-2 text-xs text-white dark:bg-zinc-100 dark:text-zinc-950" onClick={() => void exportPdf()}><Download className="size-4"/>最终导出</button></div>
    </div>
