...
export function tryAutoMergeSeparatedExamForRun(runId: string) {
  const runRow = db.prepare('SELECT batch_id FROM pdf_slicer_runs WHERE run_id = ?').get(runId) as { batch_id: string } | undefined
...
export function tryAutoMergeSeparatedExam(batchId: string) {
  const runs = db.prepare('SELECT * FROM pdf_slicer_runs WHERE batch_id = ?').all(batchId) as any[]
  ...
  `).all(batchId) as any[]
  const solutionRows = db.prepare('SELECT * FROM pdf_slicer_solution_items WHERE batch_id = ? ORDER BY created_at ASC').all(batchId) as any[]
  const solutionsByNo = new Map<string, any[]>()
