          ...
          <div className="flex-1 overflow-y-auto min-h-0 pr-1 space-y-4 pb-4">
            ...
            {activeTab === 'classification' && (
              <div className="space-y-4">
                ...
                <div className="grid gap-3">
                  <label className="space-y-1 block"><span className="text-xs text-zinc-500 font-medium">分类/清洗 API 地址</span><input className="w-full rounded-xl border px-3 py-2 text-sm" value={draft.cleanupApiBaseUrl ?? ''} onChange={(e) => setDraft({ ...draft, cleanupApiBaseUrl: e.target.value })} placeholder="留空时沿用 OCR API 地址" /></label>
                  <label className="space-y-1 block"><span className="text-xs text-zinc-500 font-medium">分类/清洗 API Key</span><input className="w-full rounded-xl border px-3 py-2 text-sm" placeholder={data?.cleanupApiKeyConfigured ? '已配置，留空不修改' : '留空时沿用 OCR API Key'} value={draft.cleanupApiKey ?? ''} onChange={(e) => setDraft({ ...draft, cleanupApiKey: e.target.value })} type="password" /></label>
                  <label className="space-y-1 block"><span className="text-xs text-zinc-500 font-medium">分类/清洗模型</span><input className="w-full rounded-xl border px-3 py-2 text-sm" value={draft.cleanupModel ?? ''} onChange={(e) => setDraft({ ...draft, cleanupModel: e.target.value })} placeholder="留空时沿用 OCR 模型" /></label>
                  <label className="space-y-1 block"><span className="text-xs text-zinc-500 font-medium">分类/清洗并发（1-20）</span><input className="w-full rounded-xl border px-3 py-2 text-sm" value={draft.cleanupConcurrency ?? ''} onChange={(e) => setDraft({ ...draft, cleanupConcurrency: e.target.value })} type="number" /></label>
                </div>
                <div className="grid gap-3 sm:grid-cols-2">
