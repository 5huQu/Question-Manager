...
const optionClass = (active: boolean) => `rounded-md px-2 py-1 text-[11px] font-medium transition-all duration-150 active:scale-[0.97] ${active ? 'border border-zinc-200/20 bg-white text-zinc-900 shadow-sm dark:bg-zinc-950 dark:text-zinc-50' : 'text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-300'}`

export function ManualFixViewerToolbar(props: Props) {
  return (
    <div className="flex shrink-0 flex-wrap items-center justify-between gap-2 border-b border-zinc-200 bg-white px-4 py-2 text-xs text-zinc-500 transition-colors dark:border-zinc-800 dark:bg-zinc-950">
      ...
      <div className="flex items-center gap-2">
        <button type="button" aria-label="上一页" disabled={props.currentPage <= 1 || props.pageBrowseMode === 'continuous'} onClick={() => props.onPageChange(props.currentPage - 1)} className="rounded-md p-1 transition-colors duration-150 hover:bg-zinc-100 active:scale-90 disabled:opacity-30 dark:hover:bg-zinc-800"><ChevronLeft className="size-4" /></button>
        <span className="font-mono">{props.currentPage} / {props.maxPages} 页</span>
        <button type="button" aria-label="下一页" disabled={props.currentPage >= props.maxPages || props.pageBrowseMode === 'continuous'} onClick={() => props.onPageChange(props.currentPage + 1)} className="rounded-md p-1 transition-colors duration-150 hover:bg-zinc-100 active:scale-90 disabled:opacity-30 dark:hover:bg-zinc-800"><ChevronRight className="size-4" /></button>
      </div>
    </div>
  )
}
