...
import { MarkdownContent, plainTextLength } from '@/components/MarkdownContent'
...
import { assetUrl, figureAlt, figureCaption, figuresByUsage } from '@/utils/questionDisplay'
...
export function QuestionContent({ blocks, figures = [], className = '', prefix }: { blocks: RichBlock[]; figures?: QuestionFigure[]; className?: string; prefix?: string }) {
  ...
  return (
    <div className={`question-content ${className}`}>
      <RichContent blocks={blocks} figures={stemFigures} prefix={prefix} />
    </div>
  )
}
...
export function ChoiceOptions({ options, figures = [] }: { options: ChoiceOption[]; figures?: QuestionFigure[] }) {
  const maxLength = Math.max(...options.map((option) => plainTextLength(option.content)))
  const layout = maxLength > 44 ? 'single' : maxLength > 22 ? 'double' : 'quad'
  return (
