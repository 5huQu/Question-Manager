...
function normalizeQuestionType(value: string, stem = '', answer = '') {
  ...
  // An explicit saved type is an editor decision. Only infer a type when the
  // stored value is absent or an OCR placeholder; otherwise words such as
  // "计算" in a choice stem must not turn a manually selected multi-choice
  // question back into an open-ended one during reads or exports.
