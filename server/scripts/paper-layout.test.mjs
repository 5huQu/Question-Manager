...
assert.deepEqual(normalizePaperLayoutDraft(undefined), { version: 1, solutionPageStrategy: 'auto', questions: [] })
...
assert.equal(draft.questions[0].multiFigureLayout, 'auto')
assert.equal(normalizePaperLayoutDraft({ questions: [{ relationId: 'q-row', multiFigureLayout: 'row' }] }).questions[0].multiFigureLayout, 'row')
...
assert.equal(normalizePaperLayoutDraft({ questions: [{ relationId: 'first', order: 0 }] }).questions[0].order, 0)
...
assert.deepEqual(questionTelemetry, [{ id: 'rel-1', startPage: 1, startPageTotal: 100, endPage: 2, endPageTotal: 710.5, pageGoal: 700 }])
