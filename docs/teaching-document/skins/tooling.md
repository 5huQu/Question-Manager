...
The checker is read-only. It statically validates the default definition, stable ID, class name, uniqueness, target compatibility metadata, `printSafe: true`, and the public authoring import. A declarative custom `skin.ts` may value-import only the authoring API and a sibling `.css` file; extra local TS/JS modules are not allowed. Type-only `@/` imports remain allowed. Optional `design` metadata must be nested static literals; the checker validates its global Token IDs, typed color/spacing/radius/border values, Slot defaults/allow-lists, and Variant bindings across the full discovered Skin tree. It parses CSS with PostCSS to require every selector to start with the exact skin root class and to reject unsafe layout rules such as fixed/sticky positioning, scrolling overflow (including axis-specific values), viewport units, external or protocol-relative URLs, CSS `@import`, animation, and pagination properties. The 2B-1B pure resolver may return a scoped variable map from this trusted metadata, but the checker neither emits it nor changes rendering. Warnings do not fail the command but require manual screen/A4 review.

It also scans `presets/**/preset.ts`. Presets must default-export `defineTeachingSkinPreset({...})`, use static literals, have unique `(id, version)` identities, and bind only existing Skin-local Variants. Scoped `skin:check -- --path ...` still keeps the full Preset dependency graph available so deleting a referenced Variant cannot escape validation.
