...
function TikzComposer({
  ...
  return (
    <section className="flex min-h-[520px] min-w-0 flex-col overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-950 motion-safe:transition-[opacity,transform] motion-safe:duration-200">
      <header className="flex items-center justify-between gap-3 border-b border-zinc-100 bg-zinc-50/60 px-4 py-3 dark:border-zinc-900 dark:bg-zinc-900/20">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 divide-y divide-zinc-200 dark:divide-zinc-800 lg:grid-cols-2 lg:divide-x lg:divide-y-0">
        <div className="flex min-h-0 flex-col bg-zinc-50/40 dark:bg-zinc-900/20">
          <div className="flex items-center justify-between border-b border-zinc-200 px-3 py-2 text-xs font-medium text-zinc-500 dark:border-zinc-800"><span>源码</span><Button size="sm" variant="outline" icon={rendering ? LoaderCircle : Code2} disabled={rendering || !source.trim()} onClick={() => void render()}>{rendering ? '生成中' : '生成预览'}</Button></div>
        ...
        <div className="flex min-h-0 flex-col">
          <div className="border-b border-zinc-200 px-3 py-2 text-xs font-medium text-zinc-500 dark:border-zinc-800">预览</div>
          <div className="flex min-h-[260px] flex-1 items-center justify-center overflow-auto bg-white p-6 dark:bg-zinc-950">{preview ? <FigurePreview fallback={dataSvg(preview)} /> : <Empty text="输入源码后将显示预览" />}</div>
        </div>
      </div>
      ...
      <footer className="flex flex-wrap items-center justify-between gap-3 border-t border-zinc-100 bg-zinc-50/40 px-4 py-3 dark:border-zinc-900 dark:bg-zinc-900/10">
...
export function QuestionFigureManager({
  ...
  if (tikzOpen) return <TikzComposer questionId={question.id} optionLabels={optionLabels} existingFigure={tikzFigure} onCancel={() => { setTikzOpen(false); setTikzFigure(undefined) }} onSaved={(figure) => { const next = tikzFigure?.id ? figures.map((item) => item.id === figure.id ? figure : item) : [...figures, figure]; commit(next); setSelectedId(String(figure.id || '')); setTikzOpen(false); setTikzFigure(undefined) }} />

  return (
    <section className="flex min-h-[520px] min-w-0 flex-col overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-950 motion-safe:transition-[opacity,transform] motion-safe:duration-200">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-100 bg-zinc-50/60 px-4 py-3 dark:border-zinc-900 dark:bg-zinc-900/20">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-[minmax(9rem,0.8fr)_minmax(0,1.7fr)] divide-x divide-zinc-200 dark:divide-zinc-800">
        <aside className="min-h-0 overflow-y-auto bg-zinc-50/35 p-2 dark:bg-zinc-900/15">
        ...
        <div className="min-h-0 overflow-y-auto p-4">
          {selected ? <div className="space-y-4">
            <div className="flex min-h-64 items-center justify-center overflow-hidden rounded-xl border border-zinc-200 bg-zinc-50/50 p-4 dark:border-zinc-800 dark:bg-zinc-900/20"><FigurePreview figure={selected} /></div>
