...
export function PropertiesSheet(props: {
  ...
  onOpenQuestionPicker?: (blockId: string) => void
}) {
...
function SheetBody(props: {
  ...
  if (block.type === 'question') {
    return <QuestionSettings {...props} block={block} question={props.question} />
  }
  ...
  return (
    <div className="space-y-3">
      <div className="space-y-1.5">
        ...
        <button
          type="button"
          onClick={() => props.onOpenQuestionPicker?.(block.id)}
      ...
      <Field label="显示编号">
        <input className={fieldClass} value={block.display?.displayNumber || ''} onChange={(event) => props.onUpdate({ display: { ...block.display, displayNumber: event.target.value } })} placeholder="如 1、例 2" />
      </Field>
