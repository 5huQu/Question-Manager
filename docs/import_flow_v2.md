# Question Manager 导入流程 V2 历史重构规划书

> 历史设计文档：本文记录 V2 形成过程，不代表当前生产路由。当前生产系统完全采用 V2；V1 PDF 切分写入口已经退役，历史数据只读保留至迁移 gate 通过。
...
该接口会为未入库候选题创建或恢复修正会话，返回来源资料信息与已有区域。前端修正工作台可据此定位题干、答案/解析和题图区域。已入库候选题不允许通过该接口继续修正。
...
> [!NOTE]
> 本节是 import flow v2 建设时使用的**历史实施计划**，保留用于理解架构演进，不代表当前功能状态。当前能力与限制以 [`capability_matrix.md`](./capability_matrix.md) 为准。
...
十六、当前已完成能力、未完成能力与已知限制（2026 年 7 月校准）

> 更细的当前能力、入口和限制统一维护在 [`capability_matrix.md`](./capability_matrix.md)。本节只保留 import flow v2 的摘要。

### 1. 已完成能力 (当前支持能力)

* **GLM-OCR 整卷识别**：
  * 支持对用户上传的整卷 PDF 原始文件和 PNG/JPG 图片格式启动异步识别轮询。
* **Doc2X v2 真实在线识别**：
  * 支持将 PDF 上传至 Doc2X v2 API、轮询解析状态、保存原始响应，并归一化为统一 `OCRDocument`。当前 Doc2X v2 入口仅接受 PDF，图片资料需使用 GLM-OCR。
* **OCRDocument 生成**：
  * 自动将 OCR 返回格式规范化为统一的 `OCRDocument` 结构，包含 markdown 文本、按页面 blocks 以及 assets 资源文件。
* **QuestionCandidate 解析**：
  * 自动切分题号、识别“题干”、“答案”、“解析”段落并生成待确认的 `QuestionCandidate`。
* **Parser 词表配置**：
  * 可动态配置切题关键字与匹配正则规则。
* **图片 DOC2X_FIGURE 占位符**：
  * 在规格化阶段同步提取 markdown 文本中的 HTML <img> 及 Markdown 格式的图片，转换为统一的 `<!-- DOC2X_FIGURE:<figureId> -->` 内联占位符。
* **远程图片本地化**：
  * 自动异步下载图片资源并持久化保存至 `data/import-flow-v2/source-documents/<sourceDocumentId>/assets/` 路径，文件名基于 URL Hash 以稳定可复现。下载失败时保留远程 URL 并生成 warning。
* **候选题确认入库**：
  * 支持在待确认页面中“一键确认入库”（支持单个/批量），将候选题入库并完美映射 figures blockId，无缝兼容现有题库，入库后题库详情页可正常内联渲染。
* **候选题手动修正**：
  * 支持从待确认候选题创建或恢复修正会话，定位原卷及分离的答案/解析资料，调整题干、解析与题图区域，编辑候选内容并保存后返回复核流程。
* **复杂题图手动调整**：
  * 手动修正工作台支持题图区域定位、重新框选、删除及题干/解析用途调整；最终效果仍需用户确认。
* **图源诊断信息**：
  * 在后端及前端“OCRDocument 候选生成”面板中展示试卷图源诊断信息（包含 Markdown 占位符数、Assets 数量、未匹配 Asset 的占位符、未使用的 Asset、下载失败图片数）。

### 2. 当前未支持能力

* **单题局部重新识别**：
  * 手动修正目前可以调整区域和直接编辑文字，但尚未提供“裁剪带上下文片段 → 调用 OCR → 新旧差异预览 → 确认写回”的完整闭环。方案见 [`region_ocr_design.md`](./region_ocr_design.md)。
* **Doc2X 单题重新识别**：
  * Doc2X 当前按整份 PDF 工作，不支持题库详情或候选题中的直接单题重跑。局部识别首版将把选区重新封装成临时 PDF 后再评估效果。
* **部分入库后的安全重解析**：
  * 已有候选题部分入库后，不应直接覆盖已入库结果。后续需要版本化重解析与明确的冲突处理策略。
