...
describe('MultiTagSelector', () => {
  ...
  it('searches options and supports checking multiple values', async () => {
    ...
    const search = container.querySelector<HTMLInputElement>('input[aria-label="搜索知识点"]')
    expect(search).not.toBeNull()
    expect(container.textContent).toContain('函数')
    expect(container.textContent).toContain('几何')
    ...
    expect(container.textContent).toContain('几何')
    expect(container.textContent).not.toContain('概率')

    await act(async () => {
      const option = Array.from(container.querySelectorAll<HTMLButtonElement>('[role="option"]')).find((button) => button.textContent?.includes('几何'))
      option?.click()
    })
  ...
  it('toggles a checked value off without losing the remaining selections', async () => {
    ...
    await act(async () => {
      const option = Array.from(container.querySelectorAll<HTMLButtonElement>('[role="option"]')).find((button) => button.textContent?.includes('函数'))
      option?.click()
    })
