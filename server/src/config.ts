...
export const uploadTempDir = path.join(storageRoot, 'tmp', 'uploads')
export const uploadLimits = {
  ...
  candidateFigureBytes: boundedInteger(process.env.CANDIDATE_FIGURE_UPLOAD_MAX_BYTES, 10 * 1024 * 1024, 1 * 1024 * 1024, 100 * 1024 * 1024),
...
function diskUpload(fileSize: number, files = 1) {
  return multer({
...
export const sourceDocumentUpload = diskUpload(uploadLimits.sourceDocumentBytes)
export const candidateFigureUpload = memoryUpload(uploadLimits.candidateFigureBytes)
export const doc2xPackageUpload = diskUpload(uploadLimits.doc2xPackageBytes)
