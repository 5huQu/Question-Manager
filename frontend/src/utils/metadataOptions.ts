...
export const provinceCityOptions: ProvinceCityOption[] = [
  ...
  { province: '新疆', cities: ['乌鲁木齐', '克拉玛依', '吐鲁番', '哈密', '昌吉', '博尔塔拉', '巴音郭楞', '阿克苏', '克孜勒苏', '喀什', '和田', '伊犁', '塔城', '阿勒泰', '石河子', '阿拉尔', '图木舒克', '五家渠', '北屯', '铁门关', '双河', '可克达拉', '昆玉', '胡杨河'] },
  { province: '香港', cities: ['香港'] },
  { province: '澳门', cities: ['澳门'] },
  { province: '台湾', cities: ['台北', '新北', '桃园', '台中', '台南', '高雄', '基隆', '新竹', '嘉义'] },
]

export const provinceOptions = provinceCityOptions.map((item) => item.province)

export type GaokaoRegionOption = {
  value: string
  label: string
  provinces: string
}

const gaokaoProvinceNames = [
...
export const gaokaoRegionOptions: GaokaoRegionOption[] = [
  ...
  {
    value: '全国三卷 / 新课标全国 III 卷',
    label: '全国三卷 / 新课标全国 III 卷',
    provinces: '',
  },
  { value: '全国甲卷', label: '全国甲卷', provinces: '' },
  { value: '全国乙卷', label: '全国乙卷', provinces: '' },
  { value: '新课标I卷', label: '新课标I卷', provinces: '' },
  { value: '新课标II卷', label: '新课标II卷', provinces: '' },
  ...gaokaoProvinceNames.map((province) => ({ value: province, label: province, provinces: '' })),
]

export function isGaokaoRegion(value?: string) {
  return Boolean(value && gaokaoRegionOptions.some((item) => item.value === value))
}

export function cityOptionsForProvince(province: string) {
  const matched = provinceCityOptions.find((item) => item.province === province)
